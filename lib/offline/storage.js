...
shaka.offline.Storage.prototype.list_ = function() {
  ...
  async function onCell(path, cell) {
    ...
    console.log("storage.js:onCell(path, cell) path", path);
    console.log("storage.js:onCell(path, cell) cell", cell);
    ...
    console.log("storage.js:onCell(path, cell) :result: ", result);
  }
  ...
  return getParser().then(function(parser) {
      ...
      .then(function(data) {
        ...
        if (isEncrypted && !includesInitData) {
          return this.extractInitData_(data, netEngine);
        } else {
          return data;
        }
      }.bind(this))
 ...
 * @private
 */
shaka.offline.Storage.prototype.extractInitData_ = async function(
  manifest, networkingEngine) {
  // get init segments uri of all variants
  /** @type {Array<Array<string>>} */
  let initSegmentReferences = [].concat.apply([],
