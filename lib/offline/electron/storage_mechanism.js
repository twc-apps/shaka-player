...
shaka.offline.electron.StorageMechanism = class {
   ...
   */
  async erase() {
    if (this.dlmStorage) { await this.dlmStorage.clear(); }
  ...
  static createV3_(storageManager) {
    ...
    const segmentStore = storageManager.segmentStore;
    const manifestStore = storageManager.manifestStore;
  ...
  static deleteAll_() {
    ...
    this.dlmStorage.manager.clear().then((resolve) => {
      p.resolve();
    }).catch((error) => {
      p.reject();
    });
