...
shaka.offline.electron.StorageMechanism = class {
  constructor() {
    ...
    /** @public {window} */
    this.dlmStorageManager = window.dlmStorageManager;
    console.log("Shaka: shaka.offline.electron.StorageMechanism, dlmStorageManager: ", this.dlmStorageManager);
  }
  ...
  init() {
    ...
    open.onsuccess = (event) => {
      ...
      this.db_ = db;
      this.v1_ = shaka.offline.electron.StorageMechanism.createV1_(db);
      this.v2_ = shaka.offline.electron.StorageMechanism.createV2_(db);
      this.v3_ = shaka.offline.electron.StorageMechanism.createV3_(db);
      p.resolve();
    };
  ...
  async erase() {
    ...
    await shaka.offline.electron.StorageMechanism.deleteAll_();
  ...
  static createV1_(db) {
    ...
    if (stores.contains(manifestStore) && stores.contains(segmentStore)) {
      ...
      return new shaka.offline.electron.V1StorageCell(
          db,
          segmentStore,
          manifestStore);
    }
  ...
  static createV2_(db) {
    ...
    if (stores.contains(manifestStore) && stores.contains(segmentStore)) {
      ...
      return new shaka.offline.electron.V2StorageCell(
          db,
          segmentStore,
          manifestStore,
