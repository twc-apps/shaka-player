...
shaka.offline.electron.V2StorageCell = class {
  ...
  async updateManifestExpiration(key, newExpiration) {
    ...
    // return op.promise();
    return await this.connection_.updateManifestExpiration(key, newExpiration);
  }
  ...
  getAllManifests() {
    ...
    //   }
    let values = {};
    values = store.getAllManifests().then(() => {
      if (values.length === 0) {
  ...
  add_(storeName, values) {
    ...
    this.connection_.generateKeyPrefix(storeName);

    /** @type {!Array.<number>} */
    let keys = [];

    values.forEach((value) => {
      this.connection_.write(storeName, value).then((resolve) => {
  ...
  remove_(storeName, keys, onRemove) {
    ...
    keys.forEach((key) => {
      this.connection_.delete(key, storeName).then(() => {
        onRemove(key);
        requests -= 1;
      });
