...
shaka.offline.electron.V2StorageCell = class {
  ...
  getAllManifests() {
    ...
    return this.manager.getAllManifests().then((response) => {
      ...
      if (response.length !== 0) {
        response.map((item) => {
          values[item.appMetadata.item.Id] = item;
        });
      }
      return values;
    });
  ...
  remove_(storeName, keys, onRemove) {
    ...
    keys.forEach((value) => {
      transactions.push(this.manager.delete(storeName, value)).then((key) => {
        onRemove(key);
      });
    });
    return Promise.all(transactions).then((keys) => {
  ...
  get_(storeName, keys) {
    ...
    keys.forEach((key) => {
      transactions.push(this.manager.get(key, storeName).then((response) => {
