...
goog.provide('shaka.offline.electron.DBConnection');

goog.require('shaka.offline.electron.DBOperation');
 ...
 */
shaka.offline.electron.DBConnection = class {
  /**
  ...
  constructor(connection) {
    ...
    /** @private {!Array.<shaka.offline.electron.DBOperation>} */
    this.pending_ = [];
  }
   ...
   *                       occur on.
   * @return {!shaka.offline.electron.DBOperation}
   */
  startReadOnlyOperation(store) {
    return this.startOperation_(store);
  }
   ...
   * @private
   */
  startOperation_(store) {
    let transaction = this.connection_.transaction([store], type);
    let operation = new shaka.offline.electron.DBOperation(transaction, store);
  ...
  /**
   * @param {!shaka.offline.electron.DBOperation} operation
   * @private
   */
