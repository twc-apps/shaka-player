...
shaka.offline.electrondb.V2StorageCell = class {
   ...
   */
  destroy() {
    // abort connection not supported for electron db
  }
  ...
  removeManifests(keys, onRemove) {
    console.log('V2StorageCell::removeManifests()', keys);
   ...
   */
  getManifests(keys) {
    console.log('V2StorageCell::getManifests()', keys);
  ...
  async getAllManifests() {
    console.log('V2StorageCell::getAllManifests()');
    ...
    for (const key in keys) {
      let transaction = await store.read(key);
  ...
  async add_(storeName, values) {
    console.log('V2StorageCell::add_()', storeName);
  ...
  async remove_(storeName, keys, onRemove) {
    console.log('V2StorageCell::remove_()', storeName, keys);
    let store = await this.db_.openIndex(storeName);
    let transactions = [];
    keys.forEach((key) => {
  ...
  async get_(storeName, keys) {
    console.log('V2StorageCell::get_()', storeName, keys);
    let store = await this.db_.openIndex(storeName);
