...
shaka.offline.electrondb.V2StorageCell = class {
  ...
  addSegments(segments) {
    return this.add_(this.segmentStore_, segments, false);
  }
  ...
  addManifests(manifests) {
    return this.add_(this.manifestStore_, manifests, true);
  }
  ...
  async updateManifestExpiration(key, newExpiration) {
    ...
    let found = await store.read(key);
   ...
   * @private
   */
  async add_(storeName, values, isManifest) {
    ...
    values.forEach((value) => {
      let transaction = store.write(value, undefined, isManifest);
      transactions.push(transaction);
    });
