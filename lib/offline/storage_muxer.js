...
shaka.offline.StorageMuxer = class {
  ...
  init() {
    ...
    console.warn("StorageMuxer::init() registry:", registry);
    MapUtils.forEach(registry, (name, factory) => {
      let mech = factory();
      console.warn("StorageMuxer::init() foreach:registry mech:", mech);
      console.warn("StorageMuxer::init() foreach:registry factory():", factory());
      if (mech) {
